...
const AddProduct = () => {
    ...
    const addProduct = async () => {
        try {
            ...
            const imageData = await uploadResponse.json(); // Parse the JSON response

            // If image upload is successful, update productDetails with the image URL
            if (imageData.success) {
                const product = {
                    ...productDetails,
                    image: imageData.image_url, // Update productDetails with the image URL received from backend
                };
                ...
                const addProductData = await addProductResponse.json(); // Parse the JSON response

                if (addProductData.success) {
                    toast.success('Product added successfully'); // Show success message
                } else {
                    toast.error('Failed to add product'); // Show error message
                }
            } else {
                toast.error('Failed to upload image'); // Show error message if image upload fails
            }
        } catch (error) {
